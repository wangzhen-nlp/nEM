    ...
    def __init__(self, data_dir, max_bags=200, max_s_len=120, mode='multi_label', flip=0.0):
        ...
        def load_dict(file):
            ...
            with open(os.path.join(data_dir, file), 'r') as f:
                ...
                id2dict = {v: k for k, v in list(dict2id.items())}
        ...
        index_list = list(range(self.n_train))
        ...
        self.test_bags, self.test_musk, self.test_pos1, self.test_pos2, self.test_labels = self.get_bags(self.create_bag('test.txt', mode))
            ...
            for line_ in f:
                ...
                for word in words:
                    if word not in worddict:
                        worddict[word] = 0
                    worddict[word] += 1
        sort_word = sorted(list(worddict.items()), key=lambda x:x[1], reverse=True)
...
class RE_Dataset(Dataset):
    ...
    def __init__(self, data_loader, dataset='train', shuffle=False):
        ...
        if dataset=='train':
            ...
            bags, musk, pos1, pos2, pos_labels = self.data_loader.test_bags, self.data_loader.test_musk, self.data_loader.test_pos1, self.data_loader.test_pos2, self.data_loader.test_labels
        labels = []
        for ls in pos_labels:
            label_rep = np.zeros(self.data_loader.n_relation, dtype=np.int_)
            label_rep[ls] = 1.
            labels.append(label_rep)
        self.index = list(range(len(bags)))
        if shuffle:
